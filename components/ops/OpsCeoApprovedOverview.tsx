...
const OpsCeoApprovedOverview: React.FC<Props> = ({ user }) => {
  const [activeTab, setActiveTab] = useState<'IDEA' | 'SCRIPT' | 'POSTED'>('IDEA');
  const [scriptFilter, setScriptFilter] = useState<'ALL' | 'WRITER' | 'CMO' | 'CEO' | 'CINE' | 'EDITOR' | 'DESIGNER' | 'OPS'>('ALL');
  ...
  useEffect(() => {
    const fetchAllProjects = async () => {
      try {
        ...
        // Filter to only CEO approved projects
        const ceoApprovedProjects = data?.filter(p =>
        ...
        setAllProjects(ceoApprovedProjects);
        
        // Fetch user details for assigned editors, sub-editors, and designers
        const assignedProjects = ceoApprovedProjects?.filter(p => p.assigned_to_user_id && 
  ...
  // Filter projects by type - only showing scripts since the requirement is for scripts only
  ...
  // Filter posted/completed projects
  const postedProjects = allProjects.filter(p =>
    p.status === 'DONE' ||
    p.current_stage === WorkflowStage.POSTED ||
    (p.data?.live_url && p.data.live_url.trim() !== '')
  );
        ...
        if (uniqueComments.length > 0) {
          ...
          if (userIds.length > 0) {
            ...
            userData.forEach(user => {
              if (user && user.id) {
                userMapTemp[user.id as string] = user;
              }
            });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-4xl font-black uppercase">CEO Approved Overview</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {projectsToShow.map((project) => (
              <div
                ...
                onClick={() => {
                  navigate(`/ops/ceo-approved-project/${project.id}`);
                }}
              >
...
export default OpsCeoApprovedOverview;