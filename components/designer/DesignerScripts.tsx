...
const DesignerScripts: React.FC<Props> = ({ project: initialProject, userRole, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
                ...
                className={`px-2 py-1 text-[10px] font-black uppercase border-2 border-black ${localProject.channel === 'YOUTUBE'
                  ...
                  : localProject.channel === 'LINKEDIN'
                    ...
                    : localProject.channel === 'INSTAGRAM'
                      ...
                      : localProject.brand === 'APPLYWIZZ_JOB_BOARD' || localProject.channel === 'JOBBOARD'
                        ? 'bg-[#00A36C] text-white'
                        : localProject.brand === 'LEAD_MAGNET_RTW' || localProject.channel === 'LEAD_MAGNET'
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        {showCinematographySection && (
          <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-bold text-slate-500 uppercase mb-2 block">{isInfluencerVideo(localProject) ? 'Shoot Video Link' : 'Shoot Video Link'}</label>
                <input
                  type="text"
                  ...
                  placeholder={isInfluencerVideo(localProject) ? "Paste link for shoot video" : "Paste link for shoot video"}
                  readOnly
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={localProject.edited_video_link || ''}
                ...
                <p className="text-xs text-slate-500 mt-1">The edited video link is managed by the editor</p>
              </div>

              {localProject.video_link && (
                <div className="pt-4 border-t-2 border-gray-200">
                  <h3 className="text-md font-bold text-slate-700 mb-2">{isInfluencerVideo(localProject) ? 'Shoot Video Reference' : 'Shoot Video Reference'}</h3>
                  <div className="bg-blue-50 border-2 border-blue-200 p-3">
                    <p className="text-sm font-bold text-blue-800 mb-1">{isInfluencerVideo(localProject) ? 'Shoot Video Link:' : 'Shoot Video Link:'}</p>
                    <a
        ...
        {/* Creative Assets Section - Only for DESIGNER role */}
        {userRole === Role.DESIGNER && (
          <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
            <div className="flex items-center gap-2 mb-4">
              <Palette className="w-5 h-5" />
              <h2 className="text-xl font-black uppercase">Creative Assets</h2>
            </div>

            <div className="space-y-4">
              <div>
                <label className="text-sm font-bold text-slate-500 uppercase mb-2 block">Thumbnail Link</label>
                <input
                  type="text"
                  value={localProject.thumbnail_link || ''}
                  className="w-full p-2 border-2 border-black font-medium focus:bg-yellow-50 focus:outline-none"
                  placeholder="Paste Google Drive link for thumbnail"
                  readOnly
                />
              </div>

              <div>
                <label className="text-sm font-bold text-slate-500 uppercase mb-2 block">Creative Assets Link</label>
                <input
                  type="text"
                  value={localProject.creative_link || ''}
                  className="w-full p-2 border-2 border-black font-medium focus:bg-yellow-50 focus:outline-none"
                  placeholder="Paste Google Drive link for creative assets"
                  readOnly
                />
              </div>
            </div>
            ...
            <div className="space-y-6 bg-white border-2 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              {comments.map((comment, index) => {
                ...
                switch (comment.stage) {
                  ...
                  case 'CINEMATOGRAPHY':
                    if (comment.action === 'SUBMITTED') {
                      description = isInfluencerVideo(localProject) ? 'Shoot video uploaded by cinematographer' : 'Shoot video uploaded by cinematographer';
                    }
                    break;
                  case 'VIDEO_EDITING':
                    if (comment.action === 'SUBMITTED') {
...
export default DesignerScripts;