...
const CineScripts: React.FC<Props> = ({ project: initialProject, userRole, onBack }) => {
  ...
  const handleSaveCinematographyInfo = async () => {
    try {
      // Get user session
      ...
      // Update the project with cinematography instructions
      const updatedData = {
        ...localProject.data,
      ...
      alert('Cinematography instructions updated successfully!');
    } catch (error) {
      console.error('Failed to update cinematography instructions:', error);
      alert('Failed to update cinematography instructions. Please try again.');
    }
  };

  const handleSaveRawVideoLink = async () => {
    try {
      // Get user session
      const { data: { session } } = await supabase.auth.getSession();
      const user = session?.user;
      ...
      // Update the project with raw video link
      const updatedData = {
        ...localProject.data,
        raw_video_link: rawVideoLink
      };

      await db.projects.update(localProject.id, {
        data: updatedData,
        video_link: rawVideoLink  // Also update the main video_link field
      });
      ...
      alert('Raw video link updated successfully!');
    } catch (error) {
      console.error('Failed to update raw video link:', error);
      alert('Failed to update raw video link. Please try again.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* Cinematographer Instructions Section - Only for CINE role */}
          ...
          <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="p-2 border-2 border-black font-medium bg-slate-50">
                  {localProject.data?.camera_angles || 'e.g. Medium shot, close-up, over-the-shoulder'}
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Completed Video Link Section - Shown when project has been moved to editor */}
        {localProject.current_stage && ['VIDEO_EDITING', 'SUB_EDITOR_ASSIGNMENT', 'SUB_EDITOR_PROCESSING', 'THUMBNAIL_DESIGN', 'FINAL_REVIEW_CMO', 'FINAL_REVIEW_CEO', 'OPS_SCHEDULING', 'POSTED'].includes(localProject.current_stage) && (
          <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
            <div className="flex items-center gap-2 mb-4">
              <Video className="w-5 h-5" />
            ...
            <div className="space-y-4">
              <div className="bg-green-50 border-2 border-green-400 p-3">
                <p className="text-sm font-bold text-green-800 mb-1">✓ Video Uploaded</p>
                {localProject.edited_video_link ? (
                  ...
                  <p className="text-gray-500 italic">Video link not yet available</p>
                )}
              </div>
              <p className="text-sm text-slate-600">Project has been moved to Editor for video editing</p>
            </div>
          </div>
        )}
