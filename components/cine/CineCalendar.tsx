...
const CineCalendar: React.FC<Props> = ({ projects = [] }) => {
    ...
    const getShootsForDay = (date: Date) => {
        const dateStr = format(date, 'yyyy-MM-dd');
        return shootDates.filter(sd => toDateKey(sd.date) === dateStr).map(sd => sd.project);
    };
        ...
        <div className="space-y-8 animate-fade-in relative">
            ...
            <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-6">
                ...
                <div className="grid grid-cols-7 gap-2">
                    ...
                    {daysInMonth.map(date => {
                        const dayProjects = getShootsForDay(date);
                            ...
                            >
                                ...
                                <div className="w-full space-y-0.5 overflow-hidden">
                                    {dayProjects.slice(0, 3).map(project => (
                                        <div key={project.id} className="bg-blue-600 text-[7px] text-white px-1 py-0.5 rounded font-black uppercase truncate w-full text-center">
                                            {project.title}
                                        </div>
                                    ))}
                                    {dayProjects.length > 3 && (
                                        <div className="text-[8px] text-blue-600 font-black text-center pt-0.5 border-t border-dashed border-blue-200">
            ...
            {selectedDay && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    ...
                    >
                        <div className="bg-blue-600 p-4 border-b-4 border-black flex items-center justify-between">
                            <h3 className="text-xl font-black text-white uppercase tracking-tight">
                                Shoots for {format(selectedDay.date, 'MMM dd, yyyy')}
                            </h3>
                            <button
                        ...
                        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-3 bg-slate-50">
                            {selectedDay.projects.map(project => (
                                <div
                                    key={project.id}
                                    className="p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-2 bg-blue-50"
                                >
                                    <div className="flex justify-between items-start">
                                        <span className="text-[10px] font-black uppercase px-2 py-0.5 border-2 border-blue-600 bg-blue-600 text-white rounded">
                                            Shoot
                                        </span>
